...
class UserEdit: UIViewController {
    ...
    private func layout() {
        ...
        let stackview5 = UIStackView(arrangedSubviews: [headerLabel, stackview1, stackview2, stackview3, stackview4, stackview6, saveButton])
        stackview5.distribution = .fillEqually
        stackview5.spacing = 20
        stackview5.axis = .vertical
        
        view.addSubview(stackview5)
        stackview5.anchor(top: view.safeAreaLayoutGuide.topAnchor, left: view.safeAreaLayoutGuide.leftAnchor, right: view.safeAreaLayoutGuide.rightAnchor, paddingTop: 10, paddingLeft: 40, paddingRight: 40, height: 500)
    }
    ...
    @objc private func handleSave() {
        ...
        Auth.auth().updateUser2(withUID: Auth.auth().currentUser!.uid, firstName: firstName, lastName: lastName, bio: bio, instagram: instagram) { (err) in
            if err != nil {
                return
            }
